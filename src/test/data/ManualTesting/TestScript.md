...
- Before you load the sample data, please make sure that the sample data file is in the same directory as the `.jar` file.
- Now let's run TaskCrusher. It is programmed to load default sample data at the start, so you should already see data inside. However, for this time let us load `SampleData.xml`.
...
`list d/today to 2017-6-10`

Expected result: only the tasks whose deadline falls within today to 2017-6-10, and events whose timeslot(s) overlaps with this range are displayed.
...
Expected result: event 19 tree planting would now be marked as incomplete and put back to the top of the list, since this is already overdue.
...
Expected result:  Task 1 in the task list will now have the new description "this must be done" and priority = 3.

`edit e 10 l/To be confirmed`

Expected result:  Event 10 in the event list will now have the new location "to be confirmed".
...
* type `undo` to undo the `clear` command If you wish to recover the data.
